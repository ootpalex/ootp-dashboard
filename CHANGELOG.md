...
- **`model/tools/split_profile.py`** — the same breakdown league-wide from the command line: `python3 model/tools/split_profile.py [--league <slug>]`. Reports every league by default, split by hitters/pitchers, overall / AA+AAA+MLB / by handedness, with the mixed players named. Also reports **how far the reversed attribute swings** — a
reversal of more than a single 5-point grade is rarer than being mixed at all: across the
shipped default league's 11,727 players, every one of the 17 mixed hitters reverses by
exactly one grade, and only one pitcher reverses by two. Reads the raw scout CSVs; read-only.
...
- **OOTP 27 support — a league can now be evaluated with OOTP 27's own regression coefficients.** Set `"ootpVersion": "27"` in `leagues/<slug>/league.json` and the pipeline routes that league through a separate, independently calibrated coefficient set. This is the largest change since 0.2.0.
  - **Version-keyed constants, not a re-fit (approach PD3b).** OOTP 27 ships as uniform `DEFAULT_*_DP_27` constants in `model/src/data_points.py`, transcribed from the calibration workspace — **all 43 locked rating→stat relationships**. The 27 path deliberately never reads `regressions_dir`: re-fitting from sims would collapse the multi-knot / off-50 structure back to a single segment and discard the calibration.
  - **`PiecewiseCoeffs` architecture** (`data_points.py`) replaces the assumption that every relationship bends at display 50. Coefficients now carry their own knot locations, so a curve can bend where the data says it bends — including multi-knot fielding with a corner clamp, and multivariate + multi-knot baserunning.
  - **Per-league version routing** in `export.py`, dispatching on `settings.ootp_version`.
  - **Displayed Stuff is capped at 88 for OOTP 27** before the regression (decision D24).
  - **OOTP 26 is byte-identical.** Every change dispatches on `PiecewiseCoeffs` or an explicit version. The 110-case `test_regressions.py` gate runs at 0.00% tolerance and is the hard guarantee that no 26 league moved. OOTP 27 has its own 124-case `test_regressions_27.py`.
  - **Audit record:** [`docs/AUDIT_27.md`](docs/AUDIT_27.md) — the audit-first trace of every input, plus end-to-end spot checks of the public pipeline against by-hand piecewise arithmetic, and a WAR smell test run through the real production 27 routing.
  - Coefficients are calibrated on the hybrid (H-pool) substrate; the constants and calibration averages were re-locked on a de-quantized display frame before shipping.
  - Validation no longer prints the "no regressions dir" note for versions that use hardcoded coefficients.
- **Fielding pipeline Phase 1 — OOTP-27 engine-boundary correctness (C1–C4).** Fixes the 2B / middle-infield overvaluation, whose root cause was three multiplying factors: stale OOTP-26 chance counts, a convexity centring bias, and a real-vs-sim response gap.
  - **C1 — engine-boundary metadata.** Metadata season directories are now filtered by engine version. `league.json` declares `engineFirstSeason` (e.g. `{"27": 2043}` for SSB); a 27 league with undeclared, flat, or pre-boundary-only metadata now fails loudly with `MetadataVersionError` instead of silently pooling previous-engine constants. Metadata cache v7.
  - **C2 — fit provenance + build-time gate.** `FIELDING_FIT_ENV_27` records each 27 curve's fit-environment chance counts and averages; `check_fielding_conversion_27` prints a fit-vs-deploy table during the build and hard-stops on >20% drift from the real-27 deployment anchors.
  - **C3 — build-time centring.** Fielding metadata now carries IP-weighted rating distributions (same population and weighting as the `avg_*` anchors), and `compose` sets each piecewise channel's constant to `−E_w[Σδ]`, making population-mean `runsP` zero by construction for any curve shape. A missing distribution warns loudly and leaves the channel untouched.
  - **C4 — governed response factors.** Per-position response factors on the 27 PM channel (2B = 0.80, replicated in both SSB and BLM; all others 1.0), with the two-sample governance rule documented at the constants.
  - The OOTP 26 path stays byte-identical — every change dispatches on `PiecewiseCoeffs` or an explicit version, and the 110-case 26 gate runs at 0.00% tolerance.
- **Standing fielding referee** (`model/tools/referee_fielding.py`) — `python3 tools/referee_fielding.py --league SSB --year 2043`. Regresses observed range runs on the model's range channel per position against a real season (chance-weighted, with HC1 / cluster / binomial-floor standard errors), reporting population means and elite bands. This is the standing arbiter for whether a response factor should move; governance requires two-sample evidence, shrunk toward 1.
- **League engine-version converter** (`model/tools/convert_league_version.py`) — one-command 26→27 conversion: `python3 tools/convert_league_version.py --league <slug> --to 27 --first-season <year> --scout-ratings <dump.csv.gz> [--dry-run]`. Fetches the season's stats from the league's StatsPlus API, derives the full metadata year-directory with verified column mappings, declares `engineFirstSeason` in `league.json`, and validates through the production loaders plus the C2 gate. Warns loudly on partial seasons and stale ratings vintages. Supersedes the older ad-hoc `derive_ratings_metadata.py` script and fixes its two mapping bugs (BA from BABIP rather than Contact; catcher ABI from CBlk rather than the C position rating). Used for the real BLM-ATL conversion.
- **Draft Board without a CSV export — StatsPlus `/draftpool` ingestion** (`model/src/draftpool.py`). When no `draft<YYYY>.csv` exists, the load path fetches the league's tokenless `/draftpool/` endpoint and builds full export-format player rows for those IDs from the league's ratings dumps (`leagues/<slug>/ratings/ratings_scout_<date>.csv.gz`, plus optional OSA, newest wins, 0.8/0.2 blend on the rating whitelist). Column mapping was verified cell-by-cell against 840 overlapping SSB `draft2043` players. `csvPresence.hasDraft` becomes "CSV present OR API pool loaded", so the sidebar entry appears with no frontend change. Hand-exported draft CSVs always win; fetch failures warn and leave the view hidden. League-generic.
- **Waiver Wire page (frontend)** — new sidebar view listing every player on waivers league-wide, with the claim-side context a decision needs: positional-need flags against your org, the same 4-toggle smart rank as the FA Finder, inherited salary / contract years / options remaining, the StatsPlus days-left clock, and your 40-man occupancy (a claim costs a spot). Frontend-only — the waiver flags were already in the data (`org.csv`'s `WAIV` column and the StatsPlus `/players` merge), so no pipeline change and no extra OOTP export is needed. The board lists **claimable players only**: `days_on_waivers_left === 0` means the player cleared waivers and can no longer be claimed, so those rows split onto their own "Cleared Waivers" table (this is what makes the count agree with OOTP's — BLM-ATL flags 65, of which 58 are claimable). New `utils/waivers.js` resolves the two disagreeing waiver signals: **StatsPlus is authoritative** (it is live as of the pipeline run and the only source carrying the claim clock), with `WAIV`-only rows shown in a separate "may be stale" section, since waivers clear in ~3 in-game days. Leagues with no StatsPlus URL configured fall back to the CSV column and the view relabels itself accordingly. The page carries no `csvPresence` gate — both signals ride in files that always exist when a dashboard builds.

### Changed

- **Fielding range target reverted: OAA → made/TOTAL (PM%).** Version 0.2.0 shipped OAA (difficulty-adjusted outs above average) as the rating→range target. The reversal was decided and written up on 2026-06-28 and landed in this repo on 2026-07-01 (commit `8af5fc9`), after a tail-bias finding: OAA's slope tracks made/TOTAL's plus an endogenous-difficulty corner hump, so the difficulty adjustment was partly re-encoding the baseline it was measured against. The target is now **made/TOTAL over BIZ buckets** (not Plays-Made/Attempts, which was also tested and rejected — it does not fix the RF/CF zone-difficulty mix). Regression cache bumped to v4 to invalidate OAA-era coefficients. Rationale: `analysis/oaa-fielding-model/docs/FIELDING_DENOMINATOR_DECISION.md` (research workspace, outside this repo).
- **Build performance and correctness pass** — roughly 4–5× faster builds. `export.py` was rewritten for speed (~37s → single digits per league); data output is now gzip-only with compressed loading in the app; per-league positional adjustments ship in `dashboard.json` as `meta.posAdj` instead of the app carrying a hardcoded BLM copy. Also: visible auto-load error messages instead of silent failure, removal of the silent SSB StatsPlus fallback for unconfigured leagues, a `leagues.json` preload hint, and the `run.py` `eval()` swap.
- **Raw OOTP pitcher exports are normalized on ingest** — `HRA` is accepted as an alias for `HRR`, and duplicate captions in a raw export are de-duplicated, so an unedited OOTP pitcher export loads without hand-fixing its header row.
- **Rule 5 Board positional needs use Now (MLB) rather than Farm.** A Rule 5 pick must stay on the active roster all season, so immediate MLB need is the correct metric for that board.
