...
import { S, posColor, levelColor, proneColor, waaStyle, devPctColor } from "../../theme.js";
...
export default function R5EligibleSubTab({ teamHitters, teamPitchers, onSelectPlayer }) {
  ...
  const r5Eligible = useMemo(() => {
    ...
    let all = [...h, ...pi];
    const R5_COL_MAP = { waaP: (p) => p.waaP, waa: (p) => p.waa, devPct: (p) => p.devPct, fv: (p) => p.fv };
  ...
  return (
    <Section title={`Rule 5 Eligible (${r5Eligible.length} players)`}>
      ...
      <div style={S.tableWrap}><table style={S.table}><thead><tr>
        {[{ key: "name", label: "Name", w: 180 }, { key: "age", label: "Age", w: 50 }, { key: "devPct", label: "Dev%", w: 48 }, { key: "pos", label: "POS", w: 50 }, { key: "bestPos", label: "Best", w: 50 }, { key: "level", label: "Level", w: 55 }, { key: "fv", label: "FV", w: 60 }, { key: "waa", label: "WAA", w: 70 }, { key: "waaP", label: "WAA P", w: 70 }, { key: "prone", label: "Prone", w: 70 }, { key: "bt", label: "B/T", w: 50 }].map(({ key, label, w }) => <SortHeader key={key} label={label} width={w} sortCol={r5Sort.col} sortDir={r5Sort.dir} colKey={key} onClick={() => toggleSort(key)} />)}
      </tr></thead><tbody>
        {r5Eligible.map((p, i) => (
