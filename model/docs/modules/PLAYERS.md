...
| As-scouted splits | `EYE vL SCOUT`, `PBABIP vR SCOUT` | Un-blended copies of the 22 vR/vL rating columns (see below) |
...
- **As-scouted split snapshots**: both blends overwrite the vR/vL rating columns in place, which loses the quantized 20-80 grade OOTP displays. `_snapshot_scouted_splits` copies those 22 columns aside under a `" SCOUT"` suffix on load, before either blend; `export.py` ships them as `ratings.scouted` for the app's L/R split-profile search. They are absent from the AAA/AA and OSA frames and from `_RATING_COLUMNS`, so no downstream blend touches them. This matters because blending two grades that are EQUAL in-game routinely lands them 1-2 points apart, which reads as a reverse platoon split that does not exist — over the default league, comparing blended ratings calls 8.1% of pitchers "mixed" against 0.6% in the raw export. Rounding a blend back to its grade does not recover it either: audited by ID against the raw export, that recovers the scouted grade for only 92.6% of hitter split ratings, because the AAA/AA relative blend can move a rating more than half a bucket.
