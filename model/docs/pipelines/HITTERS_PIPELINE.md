...
Key scaling constants: `ip_c = 1000` (catcher IP), `ip = 1200` (full-season IP). The fielding out-values `inf_out` / `of_out` are now **derived per league** (from each league's own linear weights + outfield-hit mix in `hit_aggregator`); `0.75` / `0.90` remain only as the no-data fallback. Range (PMAA) is fit against a raw **PM% = made/TOTAL** target — the `(const + slope·(rating − avg))` shape is unchanged. (An OAA target shipped 2026-05-24 and was reverted 2026-06-28 for endogenous tail bias; see `regressions.py:_range_target`.)
